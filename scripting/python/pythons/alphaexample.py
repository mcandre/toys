...
def Run():
    ...
    while not quitgame:
            ...
            Print(
                bg,
                'I want to darken the circle only.'
                ' (Move the mouse'
                ' left and right to test.)',
                (0, 10)
            )
            Print(
                bg,
                'Moving the mouse up here uses overlay method 1,'
                ' which overlays the background and circle.',
                (0, 25)
            )
            Print(
                bg,
                'This method is fast, but undesirable'
                ' because it colors the background.',
                (0, 40)
            )
            bg.fill(bgcolor2, (0, HEIGHT * 1 / 3, WIDTH, HEIGHT / 3))
            Print(
                bg,
                'Moving the mouse up here uses overlay method 2,'
                ' which overlays the circle only, but '
                'also destroys the circle\'s alpha channel.',
                (0, HEIGHT / 3 + 10)
            )
            bg.fill(bgcolor3, (0, HEIGHT * 2 / 3, WIDTH, HEIGHT / 3))
            Print(
                bg,
                'Moving the mouse down here uses overlay method 3,'
                ' which overlays the circle only, but is very slow.',
                (0, HEIGHT * 2 / 3 + 10)
            )
            Print(
                bg,
                'Do you know of a faster way to darken the bitmap,'
                ' without modifying the original picture?',
                (0, HEIGHT * 2 / 3 + 25)
            )
            ...
            if bitmappos[1] < HEIGHT / 3:
                ...
                # Method that colors only the circle
                # but destroys its alpha channel
                w, h = bitmap.get_size()
                temp = pygame.Surface(
                    (w, h),
                    pygame.SRCALPHA,
