            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == keysettings["escape"]:
                    ...
                    if event.key == keysettings["left"]:
                        ...
                        temp.bottom = 0
                        temp.left = int(
                            random() * (screenrect.width - e.width) + e.width
                        )
                        fireballrects.append(temp)
                        fireballspeeds.append((
                            0,
                            int(random() * maxfireballspeed) + 1)
                        )
        ...
        if paused:
            ...
            for i in range(len(fireballrects)):
                if fireballrects[i].colliderect(wormrect):
                    ...
                    fireballrects[i].bottom = int(
                        random() * (
                            screenrect.height - 0.75 * screenrect.height
                        )
                    )
                    fireballrects[i].centerx = int(
                        random() * (screenrect.width - 50) + 25
                    )
                    fireballspeeds[i] = (
                        0,
                        int(random() * maxfireballspeed) + 1
